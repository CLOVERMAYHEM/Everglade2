...
try {
  ...
  // Use absolute path for commands directory
  ...
  if (!fs.existsSync(commandsPath)) {
    ...
    try {
      ...
      console.error('❌ Failed to list directory contents:', listError);
    }
    process.exit(1);
  }
...
const { isUserBotAdmin } = require(path.join(__dirname, 'commands', 'stick.js'));

// Import priority tracker functions
const { createPriorityTrackerEmbed } = require(path.join(__dirname, 'commands', 'setprioritytracker.js'));
